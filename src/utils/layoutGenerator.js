...
export function generateLayout(activitiesData, dependenciesMap, activityLevelsMapUnused) {
    ...
    let dag;
    try {
        // Map dagData to [id, [parentIds]] for dagConnect
        const dagConnectData = dagData.map(({ id, parentIds }) => [id, parentIds]);
        dag = d3dag.dagConnect()(dagConnectData);
        const idToActivity = Object.fromEntries(dagData.map(({ id, _activity }) => [id, _activity]));
        dag.each(node => {
        node.data = { _activity: idToActivity[node.id] };
        });
